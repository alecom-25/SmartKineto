...
<div class="container">
    ...
    <?php if ($show_notification): ?>
        <div class="alert-rejected" style="background: #f8d7da; padding: 15px; position: relative;">
            <a href="?hide_rejected=<?php echo $rejected_request['id']; ?>"
               style="position: absolute; right: 10px; top: 5px; text-decoration: none;">&times;</a>

            <strong>Upgrade Respins:</strong> Cererea pentru <?php echo strtoupper($rejected_request['new_tier']); ?> nu
            a fost aprobată.
        </div>
    <?php endif; ?>
    ...
    <?php if ($sub): ?>
        <div class="sub-card <?php echo $sub['tier'] == 'vip' ? 'vip-gold' : ''; ?>">
            ...
            <?php if ($today > $sub['expires_at']): ?>
                <p style="color: #e74c3c; font-weight: bold;">⚠️ Atenție! Abonamentul a expirat
                    pe <?php echo date('d.m.Y', strtotime($sub['expires_at'])); ?></p>
            ...
            <?php if ($today <= $sub['expires_at']): ?>
                <ul class="benefits-list">
                    <?php if ($sub['has_fitness']): ?>
                        <li>Acces zona Fitness (Cardio & Nutriție)</li> <?php endif; ?>
                    <?php if ($sub['has_forta']): ?>
                        <li>Acces zona Forță (Greutăți)</li> <?php endif; ?>
                    <?php if ($sub['has_kineto']): ?>
                        <li>Pachet Kinetoterapie (Evaluare & Masaj Terapeutic)</li> <?php endif; ?>
            ...
            <div class="sub-actions">
                ...
                <form action="suspend.php" method="POST"
                      onsubmit="return confirm('Sigur dorești să modifici starea abonamentului?');">
        ...
        <?php if ($sub['tier'] != 'vip'): ?>
            <div class="promo-box">
                ...
                <?php if ($sub['tier'] == 'membru'): ?>
                    <p>Treci la <strong>PREMIUM</strong> pentru a combina două zone (ex: Fitness + Kineto) și
                        economisește 15% față de plata separată!</p>
                <?php elseif ($sub['tier'] == 'premium'): ?>
                    <p>Devino membru <strong>VIP</strong> pentru acces total și beneficii exclusive precum parcare
                        gratuită și prioritate la programări!</p>
                <?php endif; ?>
            </div>
        <?php endif; ?>
